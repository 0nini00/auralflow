...
export function WyCookieLoginModal({ open, onClose }: WyCookieLoginModalProps) {
  ...
  /** 递增以作废 clear 之后仍在 flight 的旧 poll */
  ...
  const clearQrPolling = useCallback(() => {
    // 先作废 in-flight poll，再清 timer，避免刷新/关窗后旧请求回写
  ...
  const startQrPolling = useCallback((key: string) => {
    clearQrPolling();
    const gen = pollGenRef.current; // clear 已 +1，此 gen 对应当前这一轮
    setQrExpired(false);
    let pending = false;
  ...
  const openNeteaseWebLogin = () => {
    ...
    if (method === "qr") {
      setError("");
      // 过期或没有 key 时重新生成，避免继续轮询死 key
      if (qrLogin && !qrExpired) {
