...
export function buildPlaybackSnapshot(source: PlaybackSnapshotSource, updatedAt = Date.now()): PlaybackSnapshot {
  ...
  // progressSampledAt 已是 wall-clock（engine 用 Date.now() 采样），直接透传即可跨 WebView 使用
  const sourceSampledAt = Number.isFinite(source.progressSampledAt) ? source.progressSampledAt : updatedAt;
  ...
  return {
    ...
    progress,
    progressSampledAt: sourceSampledAt,
    duration,
...
export function applyPlaybackSnapshotToStorePatch(snapshot: PlaybackSnapshot): PlaybackSnapshotStorePatch {
  return {
    ...
    progressSampledAt: snapshot.progressSampledAt,
