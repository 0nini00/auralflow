...
/** 登录前形状检查：至少要有 SESSDATA（DedeUserID/bili_jct 建议有，但不强制） */
export function assertBiliCookieShape(value: string): void {
  const normalized = normalizeBiliCookie(value);
  if (!normalized) throw new Error("请先粘贴 B站 Cookie");
  if (!/SESSDATA=/i.test(normalized)) {
    throw new Error("Cookie 中缺少 SESSDATA，请从已登录的 bilibili.com 请求里复制完整 Cookie");
  }
}
