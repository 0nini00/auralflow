...
/** 网易云客户端扫码登录 type。
 * type=3 生成的是「网易云音乐 App」专用码（music.163.com/login?codekey=...）。
 * 网页版「微信扫码」是另一套第三方 OAuth（微信开放平台），不是这条 client/qrcode 链路，
 * 所以微信/支付宝扫这个码通常打不开或无法授权——这是接口能力限制，不是 UI 文案问题。
 */
...
async function weapiPost(
  ...
  if (json.code !== 200) {
    ...
    if (isAuthExpiredCode(json.code)) {
      throw new Error("网易云登录已过期，请重新登录");
    }
...
function getQrMessage(code: number, fallback?: unknown): string {
  ...
  switch (code) {
    ...
    case 801:
      return "请用网易云音乐 App 扫码（微信扫不了这个码）";
    case 802:
