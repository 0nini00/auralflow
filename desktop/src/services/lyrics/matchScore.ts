...
/** 匹配分过低时视为不可信（例如歌名完全对不上） */
export const MIN_LYRIC_MATCH_SCORE = 28;

export interface RankedLyricMatch<T extends LyricSearchCandidate = LyricSearchCandidate> {
  candidate: T;
  score: number;
  index: number;
}

/** 返回按匹配分降序的候选列表（附带分数，便于多源精选） */
export function rankLyricMatches<T extends LyricSearchCandidate>(
...
/**
 * 选最高分候选；若最高分仍低于 minScore 则返回 null（避免乱配）。
 * 多源精选请用 rankLyricMatches + scoreLyricContentQuality。
 */
export function selectBestLyricMatch<T extends LyricSearchCandidate>(
  target: LyricMatchTarget,
  candidates: T[],
  minScore = MIN_LYRIC_MATCH_SCORE,
): T | null {
  const ranked = rankLyricMatches(target, candidates);
  const best = ranked[0];
